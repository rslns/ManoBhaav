...
*(embed your Loom / video link here)*
...
```
                         Webcam / Image Frame
                                  ...
                                  ▼
                 ┌──────────────────────────────┐
                 │  Router: MediaPipe face        │
                 │  detection (runs every frame)  │
                 └───────────┬─────────┬─────────┘
                   face found │         │ no face found
                              ▼         ▼
                 ┌────────────────┐   ┌──────────────────────┐
                 │  FACE BRANCH     │   │  Edge-density gate     │
                 │                  │   │  (flat-color/vector    │
                 │  Crop → 48x48    │   │   glyph check)          │
                 │  grayscale        │   └──────────┬──────────┘
                 │       ↓           │     glyph-like │ neither
                 │  MobileNetV2       │              ▼         ▼
                 │  (ImageNet,        │   ┌────────────────┐ ┌───────────┐
                 │  frozen→fine-tuned)│   │  EMOJI BRANCH    │ │ No match   │
                 │       ↓            │   │                  │ │ (nothing   │
                 │  Dense head        │   │  Resize 64x64    │ │  detected) │
                 │  (7 or 9 classes)  │   │  RGB → small CNN  │ └─────┬─────┘
                 │       ↓            │   │  (32→64→128)      │       │
                 │  Softmax → label   │   │       ↓            │       │
                 │  + confidence      │   │  Dense head        │       │
                 └─────────┬─────────┘   │  (8 classes)        │       │
                           │             │       ↓              │       │
                           │             │  Softmax → label      │       │
                           │             │  + confidence          │       │
                           ...
                           └───────────────┬──────────┴───────────────────┘
                                           ▼
                        ┌───────────────────────────────┐
                        │  Overlay: bounding box (if       │
                        │  face) + label + confidence %    │
                        │  + FPS                            │
                        └───────────────┬───────────────┘
                                        ▼
                        ┌───────────────────────────────┐
                        │  cv2.imshow (live) + optional    │
                        │  cv2.VideoWriter → output .mp4    │
                        └───────────────────────────────┘
```
...
1. **Face data:** download [FER2013 from Kaggle](https://www.kaggle.com/datasets/msambare/fer2013), extract into `data/fer2013/` so it matches `data/fer2013/train/<class>/*.jpg` and `data/fer2013/test/<class>/*.jpg`.
2. **Emoji data:** generate it — no download needed:
   ```bash
   python src/emoji_dataset_generator.py
   ```
3. *(Optional)* **Custom face classes** (crying, thinking): collect your own samples via webcam:
   ```bash
